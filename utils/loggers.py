...
def print_mean_accuracy(args, mean_acc: np.ndarray, task_number: int,
                        setting: str) -> None:
    """
...
def save_mean_accuracy_to_file(mean_acc: np.ndarray, task_number: int,
                        setting: str, args, task_id, accs, loss) -> None:
    """
    Prints the mean accuracy on stderr.
    :param mean_acc: mean accuracy value
    :param task_number: task index
    ...
    # input()
    path = os.path.join(args.output_path, args.info, 'results')
    if not os.path.exists(path):
        os.makedirs(path)
    path = os.path.join(path, str(task_id)+'.txt')
    with open(path,'w',encoding='utf-8') as f:
        print(round(loss, 8), file=f)
        print(accs[0], file=f)
        # print(accs[1], file=f)
        if setting == 'domain-il':
            mean_acc, _ = mean_acc
            print('\nAccuracy for {} task(s): {} %'.format(
                task_number, round(mean_acc, 2)), file=f)
        elif setting == 'class-il':
            mean_acc_class_il, mean_acc_task_il = mean_acc
            print('\nAccuracy for {} task(s): \t [Class-IL]: {} %'
                ' \t [Task-IL]: {} %\n'.format(task_number, round(
                mean_acc_class_il, 2), round(mean_acc_task_il, 2)), file=f)
        else:
            mean_acc_class_il, mean_acc_task_il = mean_acc
            print('\nAccuracy for {} task(s): \t [Instance-IL]: {} %\n'.format(task_number, round(mean_acc_class_il, 2)),
                file=f)
            
    path = os.path.join(args.output_path, args.info)
    if not os.path.exists(path):
        os.makedirs(path)
    path = os.path.join(path, 'output.txt')
